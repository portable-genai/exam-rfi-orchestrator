...
| # | File | What to add | Enforced by |
|---|---|---|---|
| 1 | `src/exam_rfi_orchestrator/adapters/<family>/<port>.py` | The class. Exactly one constructor shape: `def __init__(self, settings: Settings)`. Cloud SDK imports go INSIDE the method that needs them. | `test_port_parity.py::test_adapter_constructs_with_a_single_settings_argument` |
| 2 | `src/exam_rfi_orchestrator/config.py` | The `module:Class` target in `DEFAULT_BINDINGS[<port>][<profile>]`. | `test_port_parity.py::test_every_port_binds_every_known_profile` |
...
| # | File | What to add | Enforced by |
|---|---|---|---|
| 1 | `src/exam_rfi_orchestrator/ports/<port>.py` | The `@runtime_checkable` Protocol. Name the boundary, not the implementation. | `test_port_parity.py` conformance tests |
| 2 | `src/exam_rfi_orchestrator/ports/__init__.py` | Import it, add it to `PORT_PROTOCOLS` and to `__all__`, so there is one import site. | the drift guard (home 1) |
| 3 | `src/exam_rfi_orchestrator/config.py` | An entry in `DEFAULT_BINDINGS` binding ALL THREE profiles, plus one `cached_property` on `Container` that asserts the Protocol. | the drift guard (homes 2 and 4) |
| 4 | `config/settings.yaml` | The same three bindings under `adapters.<port>`. | the drift guard (home 3) |
| 5 | `src/exam_rfi_orchestrator/adapters/{local,gcp,onprem}/<port>.py` | Three adapters. `local` WORKS offline, `gcp` imports its SDK lazily, `onprem` RAISES `NotImplementedError`. A placeholder that returns successfully is a false portability claim; one that raises a bare `NotImplementedError` on a SERVING path answers 500 with no body, so raise a subclass that also carries a status and a reason (see `adapters/onprem/identity.py`). | `test_behavioral_parity.py` |
...
The five registration sites, worked through `knowledge_base` as the example, are homes 1 to 5
above: the Protocol in `ports/knowledge_base.py`, the re-export and `PORT_PROTOCOLS` entry in
`ports/__init__.py`, the `DEFAULT_BINDINGS` row plus the `Container.knowledge_base` accessor in
`config.py`, the three bindings under `adapters.knowledge_base` in `config/settings.yaml`, and the
`PortCase` in `tests/contract/canonical.py`. The three adapter families are
`adapters/local/knowledge_base.py` (answers from the fixture corpus, filters by entitlement and
reports what it suppressed as counts: a total and the same total split by artefact class),
`adapters/gcp/knowledge_base.py` (reads the governed store,
`google.*` imports INSIDE the method, refuses when the endpoint is unset or emptied because there
is no default host) and `adapters/onprem/knowledge_base.py` (raises).
...
## Walkthrough: adding a RULE to the engine

A rule that nobody can name, replay or watch fail is not a rule. Four things, and the last one is
the one people skip.

| # | What | Why |
|---|---|---|
| 1 | A rule ID, in the engine module's docstring and recorded on whatever the rule produces (`source_rule` on a requirement, `basis_rule` on a link and a withhold row, the blocker `kind`) | A reviewer and an auditor read WHY rather than inferring it years later from a status column |
| 2 | A golden case in `eval/datasets/golden_cases.jsonl` that goes RED without the rule | A rule with no case is a rule nobody has watched decide anything |
| 3 | An entry in the `BlockerKind` vocabulary, where the rule BLOCKS | The set is closed on purpose: disposition, severity and the release gate are functions of the blocker list, so a free-text reason would be a decision nobody can replay |
| 4 | A line in `SPEC.md`, plus a unit test you WATCHED go red against a deliberate defect | A guard observed only green asserts nothing |

**A new `BlockerKind` requires a matching golden case.** A blocker kind nothing exercises is a
rule nobody has watched go red, and it is indistinguishable from a typo in an enum.

If the rule changes a NUMBER (a window, a buffer, a ceiling, a floor, a cap, a tag list, a
calendar), it belongs in the `policy:` block of `config/settings.yaml` and in
`domain/policy.py`'s parser, never as a module constant. The whole point is that a firm's second
line can read it, argue with it and change it with a configuration review rather than a release.
