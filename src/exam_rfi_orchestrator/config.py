...
One env var (``EXAMRFI_PROFILE``) selects the adapter family for every
...
for a base URL, an allowlist or a path the empty string is the permissive branch), and
SET-AND-VALID wins. That refusal happens at import, which is the point: the mistake was made
seconds earlier by an operator, so it belongs in the deploy, not in production traffic.
"""
...
from .domain.models import ExamPolicy
from .domain.policy import DEFAULT_POLICY, policy_from_mapping
...
_PROFILE_ENV = "EXAMRFI_PROFILE"
_SETTINGS_ENV = "EXAMRFI_SETTINGS"
...
#: The port whose ACTIVE binding decides the model ``generator_model`` reports, and the
#: dotted ``Settings`` path holding that model's id. This service DOES bind a generative port
#: (the model drafts, classifies and normalises; it owns no number), so the banner follows the
#: binding rather than a settings string somebody has to remember to update alongside it. The
#: model attribute is named too, because the managed model id is per-deployment configuration
#: and a banner naming a default the deployment never calls is worse than one that says unset.
_GENERATOR_PORT: str = "generation"
_GENERATOR_MODEL_ATTR: str = "generation_model"
...
class ProfileChoice:
    ...
    The variable is ``EXAMRFI_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("EXAMRFI_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "knowledge_base": {
        "local": f"{_PKG}.adapters.local.knowledge_base:LocalKnowledgeBaseAdapter",
        "gcp": f"{_PKG}.adapters.gcp.knowledge_base:CloudKnowledgeBaseAdapter",
        "onprem": f"{_PKG}.adapters.onprem.knowledge_base:OnPremKnowledgeBaseAdapter",
    },
    "obligations": {
        "local": f"{_PKG}.adapters.local.obligations:LocalObligationsAdapter",
        "gcp": f"{_PKG}.adapters.gcp.obligations:CloudObligationsAdapter",
        "onprem": f"{_PKG}.adapters.onprem.obligations:OnPremObligationsAdapter",
    },
    "evidence_packs": {
        "local": f"{_PKG}.adapters.local.evidence_packs:LocalEvidencePackAdapter",
        "gcp": f"{_PKG}.adapters.gcp.evidence_packs:CloudEvidencePackAdapter",
        "onprem": f"{_PKG}.adapters.onprem.evidence_packs:OnPremEvidencePackAdapter",
    },
    "generation": {
...
def _expand(value: str) -> str:
    ...
    ``${VAR:-default}`` is ``setting_or_default(VAR, default)`` one layer down, so it delegates
    to that helper and there is exactly one implementation of the rule. A variable that is
    present and EMPTY raises rather than resolving to the empty string: an operator who emptied
    it named nothing, and for a base URL, an allowlist or a path the empty string is the
    permissive branch. ``${VAR}`` with no ``:-`` defaults to the empty string, so unset still
    resolves to empty while emptied still refuses.
    """
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``EXAMRFI_SETTINGS``) that does not
...
def _policy_block(data: Mapping[str, Any]) -> Mapping[str, Any] | None:
    """The settings file's ``policy:`` mapping, or ``None`` to take the shipped reference block."""
    block = data.get("policy")
    if block is None:
        return None
    if not isinstance(block, Mapping):
        raise ValueError("settings 'policy' must be a mapping of policy keys to values")
    return block
...
class Settings:
    ...
    #: from the wrong host is a false statement about what the firm holds.
    knowledge_base_url: str = ""
    #: The obligation register's read surface. Empty means the managed adapter refuses.
    obligations_url: str = ""
    #: The assembled evidence-pack read surface. Empty means the managed adapter refuses.
    evidence_packs_url: str = ""
    #: The Firestore collection the managed case store writes the open-item board to. Empty
    #: means the managed adapter refuses rather than writing to a default collection.
    case_collection: str = ""
    #: Where the OFFLINE case store keeps its SQLite file. ``:memory:`` is the ephemeral,
    #: deterministic default the gate and the demo use; a deployment points it at a durable path.
    case_store_path: str = ":memory:"
    ...
    #: ``policy:`` section of the settings file, so no number an exam response turns on stays a
    #: module constant.
    policy: ExamPolicy = field(default_factory=lambda: DEFAULT_POLICY)
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            knowledge_base_url=str(data.get("knowledge_base_url") or ""),
            obligations_url=str(data.get("obligations_url") or ""),
            evidence_packs_url=str(data.get("evidence_packs_url") or ""),
...
class Container:
    ...
    @cached_property
    def knowledge_base(self) -> KnowledgeBaseReadPort:
        adapter = self._bind("knowledge_base")
        assert isinstance(adapter, KnowledgeBaseReadPort)
        return adapter

    @cached_property
    def obligations(self) -> ObligationsReadPort:
        adapter = self._bind("obligations")
        assert isinstance(adapter, ObligationsReadPort)
        return adapter

    @cached_property
    def evidence_packs(self) -> EvidencePackReadPort:
        adapter = self._bind("evidence_packs")
        assert isinstance(adapter, EvidencePackReadPort)
        return adapter
