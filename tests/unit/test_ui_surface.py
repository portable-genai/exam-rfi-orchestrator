...
@requires_ui
def test_withheld_denied_and_missing_are_rendered_visually_distinct() -> None:
    """A UI test rather than a convention, because rendering them alike is THE failure.

    "Withheld under privilege", "you are not entitled to see this" and "the firm holds no such
    document" are materially different statements to a supervisor, and only one of them is true
    in any given row. The whole ACL design exists to keep them apart, and a console that painted
    them the same colour would undo it on the one surface a person actually reads.
    """
    css = (UI / "app" / "globals.css").read_text(encoding="utf-8")
    page = (UI / "app" / "page.tsx").read_text(encoding="utf-8")
    for state in ("withheld", "denied", "missing"):
        assert "tr.cov." + state in css, "coverage state " + state + " renders like every other"
    for status in ("withheld_privileged", "withheld_sar", "denied_no_entitlement"):
        assert "tr.wh." + status in css, "withhold status " + status + " renders like the rest"
    # The rules must actually DIFFER, or three selectors painting one colour would pass above.
    rules = {
        state: css.split("tr.cov." + state + " td {", 1)[1].split("}", 1)[0].strip()
        for state in ("withheld", "denied", "missing")
    }
    assert len(set(rules.values())) == 3, "two coverage states are painted identically: " + str(
        rules
    )
    # And the class has to reach the row, or the stylesheet is decoration.
    assert '"cov " + row.state' in page
    assert '"wh " + row.status' in page


@requires_ui
def test_the_console_says_it_cannot_release_anything() -> None:
    """The one sentence an audience must never have to ask about."""
    page = (UI / "app" / "page.tsx").read_text(encoding="utf-8")
    assert "cannot release anything" in page
    assert "held_for_checker" in page or "release_state" in page


@requires_ui
def test_the_console_labels_a_waiver_and_an_extension_as_lookup_keys() -> None:
    """A field labelled "waiver" invites an operator to think typing one grants something."""
    page = (UI / "app" / "page.tsx").read_text(encoding="utf-8")
    assert page.count("a lookup key into the case store; it grants nothing") == 2
