...
def test_the_demo_data_is_obviously_fictional() -> None:
    items = (
        demo.ROUTINE_ITEM,
        demo.ESCALATING_ITEM,
        demo.PII_ITEM,
        demo.SKILLED_PERSON_ITEM,
    )
    # The supervisor is an invented authority that announces itself in its own name, because a
    # demo regulator with the initials of a real one is the kind of fiction nobody double-checks.
    assert "FICTIONAL" in demo.REGULATOR
    assert "FICTIONAL" in demo.REFERENCE and "FICTIONAL" in demo.SKILLED_PERSON_REFERENCE
    for prior in demo.PRIOR_ANSWERS:
        assert "FICTIONAL" in prior.assertion_value, "a prior answer names a real-looking party"
    blob = " ".join(item.question for item in items) + demo.ACTOR
