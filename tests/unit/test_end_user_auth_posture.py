...
read
    ...
    bind_profile == local AND profile chosen AND EXAMRFI_S2S_TOKEN unset
...
approver's groups and tenant, and ``/v1/response-pack`` with a real, routed response pack
written to the review outbox.
...
from exam_rfi_orchestrator.adapters.local.identity import (
    LocalIdentityAdapter,
)
from exam_rfi_orchestrator.adapters.onprem.identity import (
