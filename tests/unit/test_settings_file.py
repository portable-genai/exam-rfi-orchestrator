...
def test_an_env_var_set_to_empty_does_not_inherit_the_files_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Three-state expansion: unset takes the default, set-and-empty REFUSES.

    The loader delegates ``${VAR:-default}`` to ``envread.setting_or_default``, so the settings
    file obeys the same rule as every adapter. Resolving an emptied variable to the empty string
    would make ``${VAR:-http://audit:8080}`` with ``VAR=""`` indistinguishable from ``${VAR:-}``,
    and for a base URL, an allowlist or a path the empty string is the permissive branch.
    """
