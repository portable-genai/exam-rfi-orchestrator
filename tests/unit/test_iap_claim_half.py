...
def test_a_claim_set_from_another_issuer_cannot_become_a_principal() -> None:
    """Refused twice, and both refusals are asserted.

    ``resolve`` states the issuer itself, because ``verify_token`` does not check it. The claim
    half states it again. Defence in depth is only defence if both halves are checked, so the
    commons is also called directly with the same claim set: a token that satisfied a signature
    check and named somebody else as its issuer must produce NO principal on either path,
    rather than a verified identity built out of its ``email``.
    """
    bad = _claims(iss="https://accounts.google.com")
    with pytest.raises(IdentityError):
        _resolve(bad)
    with pytest.raises(IdentityError):
        principal_from_iap_claims(
            bad, _FEDERATION_POLICY, source="gcp-iap", include_subject_principal=False
        )
...
def test_a_verified_assertion_that_names_nobody_is_refused() -> None:
    ...
    with pytest.raises(IdentityError):
        principal_from_iap_claims(
            blank, _FEDERATION_POLICY, source="gcp-iap", include_subject_principal=False
        )


def test_the_address_is_required_here_even_though_the_commons_does_not_require_it() -> None:
    """A divergence kept on purpose, and stated where somebody would otherwise remove it.

    ``principal_from_iap_claims`` needs only a subject and falls back to the opaque ``sub``.
    A rendered service attributes its audit records to the address, so an assertion carrying
    ``sub`` alone is refused before the claim half runs rather than resolved to an id nobody
    can recognise in a review queue.
    """
    with pytest.raises(IdentityError, match="requires both sub and email"):
        _resolve(_claims(email=None))
    assert (
        principal_from_iap_claims(
            _claims(email=None),
            _FEDERATION_POLICY,
