...
from exam_rfi_orchestrator.adapters._review_payload import (
    result_to_review,
)
...
# --------------------------------------------------------------------------- #
def _service(container: Any) -> ResponsePackService:
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    """The console is a shared sink, so this holds for every family, not only the demo's."""
    service = _service(build_container(local_settings()))
    ...
    payload = repr(
        result_to_review(
            assessment, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT
        ).to_payload()
    )
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    assessment = _service(local).assess_item(
        sample_cases.REQUEST,
    ...
    assert assessment.citations, "an offline answer must still be cited"
    assert local.review_router.route(assessment, maker=sample_cases.ACTOR)

    onprem = build_container(local_settings(profile="onprem"))
    with pytest.raises(NotImplementedError):
        _service(onprem).assess_item(
            sample_cases.REQUEST,
            sample_cases.ESCALATING_ITEM,
            actor=sample_cases.ACTOR,
            tenant=sample_cases.TENANT,
