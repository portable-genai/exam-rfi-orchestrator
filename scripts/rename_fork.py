...
The catalog id (``Cop2``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
artefact taxonomy, the withhold ladder, the holiday calendar, region, IdP, eval golden set)
that this script does NOT do for you. In particular it does not touch the handling tags that
drive privilege and restricted-filing withholding: a fork that renames the package and keeps the
reference taxonomy has rebranded a disclosure policy its counsel has never read.
"""
...
_OLD_PACKAGE = "exam_rfi_orchestrator"
_OLD_ENV_PREFIX = "EXAMRFI_"
_OLD_BARE_PREFIX = "EXAMRFI"
_OLD_RESOURCE = "cop2-svc"
_OLD_DIST = "exam-rfi-orchestrator"
_OLD_CATALOG_ID = "Cop2"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (EXAMRFI_PROFILE), never mid-word.
    ...
    # The bare token in BACKTICKED prose (AGENTS.md and docs/practices-audit.md each identify
    # this repo as "env `EXAMRFI`"). Without this, a fork run with --include-docs still tells
    # its own agents to set the upstream prefix. It stays safe for the same reason the line
    # above is: a backticked whole token is a code span naming the variable prefix, not a word
    # that could appear inside prose by accident.
    text, changed = re.subn(rf"`{re.escape(_OLD_BARE_PREFIX)}`", f"`{env_prefix}`", text)
    count += changed
    return text, count
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-rfi",
    )
    ...
    print("Then finish the human steps in docs/ADOPTING.md (taxonomy, withhold ladder, IdP).")
    return 0
