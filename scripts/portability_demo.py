...
from exam_rfi_orchestrator import (
    config,
)
from exam_rfi_orchestrator.domain import (
    kernel,
)
from exam_rfi_orchestrator.ports import (
    PORT_PROTOCOLS,
)
...
def check_the_exit_family_refuses() -> str:
    ...
    for port, call in demo.EXIT_CALLS.items():
        # A seam in EXIT_ABSENT is asserted the OTHER way round: it must complete. Tracing is a
        # diagnostic, not a control, and making it fatal on-prem would oblige every operator to
        # stand up a tracing stack before the service would answer a request. Asserting it here
        # rather than exempting it is what makes a tracer that starts raising fail this tour.
...
def check_the_trail_leaves_this_codebase_intact() -> str:
    with tempfile.TemporaryDirectory(prefix="portability-") as work:
        ...
        # The export leads with the anchor header and then carries one line per record, so the
        # file is one line LONGER than the record count export_jsonl() returns. Header and record
        # are told apart by key, not by position: a header carries "anchor" and no "event".
        ...
        if len(lines) != written + 1 or "anchor" not in header or "event" in header:
            raise AssertionError("the export does not lead with one anchor header per record set")
        if header.get("format") != EXPORT_FORMAT:
            raise AssertionError("the export header does not name the documented export format")
        ...
        # Why the header has to travel: a shortened export is internally PERFECT, so the chain
        # alone cannot see a dropped tail. The anchor is what the recipient checks the last
        # record against, so dropping one record now has to be refused rather than restored.
        # Rebuilt from the RAW lines, since re-encoding the parsed JSON would not be the bytes
        # the hashes were taken over.
    ...
    return str(written) + " records exported as anchored JSONL, reloaded, truncation refused"
...
def _event() -> kernel.AuditEvent:
    return kernel.AuditEvent(
        ...
        redacted_summary=demo.REFERENCE + " item 2.c: blocked",
    )
