...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden exam requests.", ""]
    return lines


def _exercised_block() -> list[str]:
    cases = load_jsonl(GOLDEN)
    planted = [row for row in cases if row.get("planted")]
    blockers = sum(len(row.get("blocker_kinds", [])) for row in cases)
    blocked = sum(1 for row in cases if row.get("blocker_kinds"))
    return [
        BLOCKS[1],
        "",
        f"- **{len(cases)} golden exam requests** in `eval/datasets/golden_cases.jsonl`, each",
        "  with the disposition, deadline, completeness verdict and blockers a reviewer assigned.",
        f"- **{blockers} blockers across {blocked} of those cases.** That is what `blocker_recall`",
        "  is micro-averaged over, and the distinction is the reason the metric means anything: a",
        f"  per-case mean gave the {len(cases) - blocked} cases with no blockers a free 1.0 each.",
        f"- **{len(planted)} cases plant a raw identifier**, so the leak metric has a target it",
