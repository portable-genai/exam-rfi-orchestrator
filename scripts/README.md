...
| Script | What it does |
|---|---|
| `run_narrative_eval.py` (in `eval/`) | Judges the response narrative against written criteria and the model-risk floors in `config/quality-floors.toml` (`make eval-narrative`, inside `make gate`). Offline: the judge is deterministic and named on the command line, never read from the environment, because a judge a deployment can swap by variable is a judge nobody reviewed. |
| `render_evals_doc.py` | Regenerates the derived half of `docs/evals.md` from the rubrics and the golden set (`make evals-doc`). `--check` runs inside `make gate`, so a bar that moved without the page being re-rendered fails the build instead of leaving a reader a page that quietly stopped being true. Only the named headings are generated; the prose around them is hand-written and untouched. |
| `demo.py` | The scripted arc, driving the REAL services over the `local` profile, behind the same eight step keys: bind the stack and show the POLICY block it will decide with, assess a fully evidenced item (deterministic, cited, indexed, not escalated), assess a consequential one and route it (rule R8), plant a national id and three non-producible documents and show masking before the audit write alongside the withholding schedule, show the checker's queue plus the exam lead's open-item board, verify and export the audit trail, rewrite a withhold basis and detect it, then swap to the exit profile and watch every seam refuse. Writes the audit-view JSON. The demo passes a FIXED `as_of` date into the service so the clocks are byte-stable, while the API resolves its own and echoes it. |
...
| `rename_fork.py` | The one-pass rebrand for a fork: the package name (which is also the console script), the `EXAMRFI_` env prefix including the bare token `infra/terraform/render.tf.json` carries and the backticked form the docs carry, the Terraform `name_prefix` resource stem (`cop2-svc`) and the distribution id. Prints a plan and writes nothing without `--yes`; skips itself, so the renamer is not left half-rewritten. It touches no handling tag, so a renamed fork carries a disclosure policy its counsel has not read. See `docs/ADOPTING.md`. |
